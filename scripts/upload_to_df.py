...
def upload_string_to_df(upload_string, filename):
    ...
    for split_entry in split_string_list[1:]:
        split_updated.append([np.array(split_check.split(',')).astype(np.float64) if ',' in split_check and test_float_cast(split_check)
                              else split_check for split_check in split_entry])
    ...
    string_frame_exploded = pd.concat([pd.DataFrame({expand_values: np.concatenate(string_frame[expand_values])
                                                     for expand_values in ['X', 'Y']})], axis=1)
