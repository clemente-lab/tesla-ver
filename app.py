...
from scripts.json_helpers import parse_loclust_string_to_json, get_X_and_Y, get_keys_metadata, get_all_Y_at_X
import csv
import httplib
import json
# misc imports
...
app.layout = html.Div([
    ...
    #     dcc.Dropdown(id = "selected-type-2", options = [{"label": i, "value": i} for i in get_keys_metadata()]), 
    # ], style = {
    # }), 
...
@app.callback(Output('barchart-main', 'children'), 
              [Input('main-slider', 'value')])
def draw_heat_map(slider_val):
    json_dict = dict()
    with open('./assets/json_data/db.json', 'r') as read_file:
        json_dict = json.load(read_file)
        x_list = list()
        for id in json_dict.keys()[1:]:
            x_list.append(json_dict.get(id).get('country_name'))
        print get_all_Y_at_X(slider_val, json_dict)
        trace_one = go.Bar(
            x = x_list,
            y = get_all_Y_at_X(slider_val, json_dict), 
        )
    print slider_val
    return {'data':trace_one, 'layout': go.Layout(
            xaxis = {
                'title': '123', 
                'type': 'linear'
                }, 
...
def update_output(file_to_upload, file_name):
    if file_to_upload is not None:
        ...
        dict_keys = [{"label": i, "value": i} for i in get_keys_metadata()]
